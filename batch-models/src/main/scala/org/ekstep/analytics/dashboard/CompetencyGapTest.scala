...
import org.apache.spark.sql.functions.{col, current_timestamp, explode, explode_outer, expr, from_json, lit, schema_of_json}
...
object CompetencyGapTest extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    implicit val spark: SparkSession =
      SparkSession
        ...
        .getOrCreate()
    val res = time(competencyGapData());
  ...
  def competencyGapData()(implicit spark: SparkSession): DataFrame = {
    ...
    gapDF = gapDF.select(
      ...
      col("org_id"),
      col("work_order_id"),
      col("competency_level"),
      col("declared_level")
    )
    gapDF = gapDF.withColumn("competency_gap", expr("competency_level - declared_level"))
