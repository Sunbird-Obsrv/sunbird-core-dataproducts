...
class Summary(val firstEvent: ReducedV3Event) {
...
//    val object_rollup: Option[RollUp] = if(firstEvent.`object`.nonEmpty) firstEvent.`object`.get.rollup else None
    ...
    var lastEvent: ReducedV3Event = null
    ...
    var lastImpression: ReducedV3Event = null
    var impressionMap: Map[ReducedV3Event, Double] = Map()
    ...
    def add(event: ReducedV3Event, idleTime: Int) {
...
//            if(this.PARENT != null) return PARENT else return this;
    ...
    def checkEnd(event: ReducedV3Event, idleTime: Int, config: Map[String, AnyRef]): Summary = {
...
//        }
    ...
    def getSimilarEndSummary(event: ReducedV3Event): Summary = {
        val mode = if(event.edata.mode == null) "" else event.edata.mode
        if(StringUtils.equalsIgnoreCase(this.`type`, event.edata.`type`) && StringUtils.equals(this.mode.get, mode)) {
            return this;
        }
