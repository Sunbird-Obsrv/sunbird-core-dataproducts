...
class TestCourseEnrollmentModel extends SparkSpec with Matchers with MockFactory {
  ...
  override def afterAll() {
   ...
   ignore should "execute Course Enrollment model" in {
  ...
  ignore should "give error if there is no data for output" in {
  ...
  ignore should "execute Course Enrollment model and dispatch to azure even if type is other than csv" in {
    implicit val mockFc = mock[FrameworkContext]
