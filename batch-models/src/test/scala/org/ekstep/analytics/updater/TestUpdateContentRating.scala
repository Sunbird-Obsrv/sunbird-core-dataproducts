...
class TestUpdateContentRating extends SparkSpec(null) with MockFactory {
  ...
  it should "get all content ratings" in {
    val mockRestUtil = mock[HTTPClient]
    (mockRestUtil.post[List[Map[String, AnyRef]]](_: String, _: String, _: Option[Map[String, String]])(_: Manifest[List[Map[String, AnyRef]]]))
      .expects("http://localhost:8082/druid/v2/sql/", AppConf.getConfig("druid.content.rating.query"), None, manifest[List[Map[String, AnyRef]]])
    ...
    val contentRatings = UpdateContentRating.getContentMetrics(mockRestUtil, AppConf.getConfig("druid.content.rating.query"))
  ...
  it should "get all content consuption metrics" in {
    ...
    (mockRestUtil.post[List[Map[String, AnyRef]]](_: String, _: String, _: Option[Map[String, String]])(_: Manifest[List[Map[String, AnyRef]]]))
      .expects("http://localhost:8082/druid/v2/sql/", AppConf.getConfig("druid.content.consumption.query"), None, manifest[List[Map[String, AnyRef]]])
      .returns(JSONUtils.deserialize[List[Map[String, AnyRef]]](response))

    val contentConsumption = UpdateContentRating.getContentMetrics(mockRestUtil, AppConf.getConfig("druid.content.consumption.query"))
